...
impl IndexError {
    ...
    pub fn hint(&self) -> Option<&'static str> {
        match self {
            Self::NotAGitRepository => Some(
                "Make sure you're inside a git repository, or use --path to specify one."
            ),
            Self::InvalidGitFile => Some(
                "The .git file may be corrupted. This can happen with broken worktree setups."
            ),
            Self::IndexNotFound => Some(
                "Run: git-semantic index"
            ),
            Self::CorruptedIndex(_) => Some(
                "The index file is unreadable. Rebuild it with: git-semantic index --force"
            ),
            Self::SaveFailed(_) => Some(
                "Check available disk space and file permissions on the .git directory."
            ),
            Self::Io(_) => Some(
